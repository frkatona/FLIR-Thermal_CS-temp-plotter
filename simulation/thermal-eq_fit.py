...
PDMS_thermal_conductivity_WpmK = 0.2  # W/mK
...
q = np.zeros((Nx, Ny, Nz))
q[beam_mask] = Q / (np.pi * beam_radius_m**2 * cube_length_m)  # power density distributed across the entire volume
q *= np.exp(-1 * abs_coeff * loading * Z)  # depth-dependent exponential decay

# Show what fraction of power extends beyond the cube
transmittance = (Q - np.sum(q * dx * dy * dz)) / Q
print(f'Transmittance through material: {transmittance:.2%}')
print(f'max q (W): {np.max(q) * dx * dy * dz:.2e}')
