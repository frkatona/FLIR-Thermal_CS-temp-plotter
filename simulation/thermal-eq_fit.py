...
TD_steel = 1.172e-25  # Thermal diffusivity of steel, m^2/s
...
def compute_T_3D_RK(output_times):
    ...
    for n in range(max(output_indices) + 1):
        k1 = dt * (TD_steel * (np.roll(T, -1, axis=0) - 2*T + np.roll(T, 1, axis=0)) / dx**2
                   + TD_steel * (np.roll(T, -1, axis=1) - 2*T + np.roll(T, 1, axis=1)) / dy**2
                   + TD_steel * (np.roll(T, -1, axis=2) - 2*T + np.roll(T, 1, axis=2)) / dz**2
                   + q)
        
        T_temp = T + k1 / 2
        k2 = dt * (TD_steel * (np.roll(T_temp, -1, axis=0) - 2*T_temp + np.roll(T_temp, 1, axis=0)) / dx**2
                   + TD_steel * (np.roll(T_temp, -1, axis=1) - 2*T_temp + np.roll(T_temp, 1, axis=1)) / dy**2
                   + TD_steel * (np.roll(T_temp, -1, axis=2) - 2*T_temp + np.roll(T_temp, 1, axis=2)) / dz**2
                   + q)
        
        T_temp = T + k2 / 2
        k3 = dt * (TD_steel * (np.roll(T_temp, -1, axis=0) - 2*T_temp + np.roll(T_temp, 1, axis=0)) / dx**2
                   + TD_steel * (np.roll(T_temp, -1, axis=1) - 2*T_temp + np.roll(T_temp, 1, axis=1)) / dy**2
                   + TD_steel * (np.roll(T_temp, -1, axis=2) - 2*T_temp + np.roll(T_temp, 1, axis=2)) / dz**2
                   + q)
        
        T_temp = T + k3
        k4 = dt * (TD_steel * (np.roll(T_temp, -1, axis=0) - 2*T_temp + np.roll(T_temp, 1, axis=0)) / dx**2
                   + TD_steel * (np.roll(T_temp, -1, axis=1) - 2*T_temp + np.roll(T_temp, 1, axis=1)) / dy**2
                   + TD_steel * (np.roll(T_temp, -1, axis=2) - 2*T_temp + np.roll(T_temp, 1, axis=2)) / dz**2
                   + q)
